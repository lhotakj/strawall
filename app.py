...
app.config.athlete_id = None

def auth():
    app.config.athlete_id = session.get('athlete_id')
    if not app.config.athlete_id:
        return redirect(url_for('login'))
    token_data = app.config.db.load_token(app.config.athlete_id)
...
def refresh_token():
    ...
    if token_data and datetime.now() > datetime.fromtimestamp(int(token_data["expires_at"])):
        ...
        app.config.db.save_token_cache(app.config.athlete_id, new_token_data)
...
def activities():
    auth()
    cached_activities = app.config.db.load_activities_cache(app.config.athlete_id)
    ...
    app.config.db.save_activities_cache(app.config.athlete_id, new_activities)
